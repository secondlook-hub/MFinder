...
final class UpdateInstaller {
    ...
    func install(from downloadURL: URL, version: String) {
        ...
        let panel = UpdateProgressPanel(title: "MFinder \(version) 업데이트")
        self.panel = panel
        panel.show(status: "다운로드 중…")
        ...
        Task {
            do {
                ...
                self.panel = nil
                let alert = NSAlert()
                alert.messageText = "자동 업데이트 실패"
                alert.informativeText = error.localizedDescription
                    + "\n\n다운로드 페이지에서 수동으로 설치할 수 있습니다."
    ...
    private func performInstall(downloadURL: URL, panel: UpdateProgressPanel) async throws {
        ...
        guard let http = response as? HTTPURLResponse, http.statusCode == 200 else {
            throw InstallError(message: "다운로드에 실패했습니다 (HTTP \((response as? HTTPURLResponse)?.statusCode ?? 0)).")
        }
        let dmg = tempFile.deletingLastPathComponent().appendingPathComponent("MFinder-update.dmg")
        try? FileManager.default.removeItem(at: dmg)
        try FileManager.default.moveItem(at: tempFile, to: dmg)
        defer { try? FileManager.default.removeItem(at: dmg) }

        // 2. Mount it.
        panel.show(status: "디스크 이미지 마운트 중…")
        ...
        guard let mountPoint = Self.mountPoint(fromAttachPlist: attachOut) else {
            throw InstallError(message: "디스크 이미지를 마운트하지 못했습니다.")
        }
        ...
        guard let appName = try FileManager.default.contentsOfDirectory(atPath: mountPoint)
            ...
            throw InstallError(message: "디스크 이미지 안에서 앱을 찾지 못했습니다.")
        }
        panel.show(status: "설치 중…")
        let mountedApp = "\(mountPoint)/\(appName)"
        let staging = FileManager.default.temporaryDirectory
            .appendingPathComponent("MFinder-update-\(UUID().uuidString).app").path
                ...
                if task.terminationStatus != 0 {
                    ...
                    continuation.resume(throwing: InstallError(
                        message: detail.isEmpty
                            ? "\(URL(fileURLWithPath: launchPath).lastPathComponent) 실행 실패 (코드 \(task.terminationStatus))"
                            : detail))
                } else {
